...
class Node:
    def __init__(self, s, model, C, pi_dim=4, verbose=False, using_prior_for_exploration=False):
        ...
        if self.verbose:
            print("NEW NODE:", np.shape(self.s), self.NODE_ID)
    ...
    def select(self, deterministic=True):
        ...
        while None not in path[-1].children_nodes:  # If not leaf!
            if deterministic:
                ...
                path[-1].in_progress = np.random.choice(self.pi_dim, p=path[-1].probs_for_selection())
    ...
    def expand(self, use_means=False, samples=1):
        ...
        for i in range(self.pi_dim):
            self.children_nodes[i] = Node(
                s=ps_next[i], model=self.model, C=self.C, pi_dim=self.pi_dim, using_prior_for_exploration=self.using_prior_for_exploration
            )
        if self.verbose:
            print("Expand:", G.numpy(), self.W, self.N)
    ...
    def backpropagate(self, path, G):
        ...
        for i in range(len(path)):
            ...
            if self.verbose:
                print("Propagating to node", path[i].NODE_ID, "with N:", path[i].N)
...
class MCTS:
    def __init__(self, C=1.0, threshold=0.5, repeats=300, simulation_repeats=1, simulation_depth=3, use_habit=False):
        self.C = C
    ...
    def active_inference_mcts(self, model, frame, o_shape=(64, 64, 1)):
        ...
        root.expand(use_means=self.use_means)

        for repeat in range(self.repeats):

            if calc_threshold(normalization(root.N), axis=0) > self.threshold:
            ...
            path[-1].expand(use_means=self.use_means)
            ...
            for sim_repeat in range(self.simulation_repeats):
                ...
                all_av_G[sim_repeat], pi0, path[-1].Qpi = model.mcts_step_simulate(path[-1].s[0], self.simulation_depth, use_means=False)
        ...
        if self.verbose:
            print(
                ...
                np.round(root.C * root.Qpi * np.sqrt(root.N.sum()) / (1.0 + root.N), 2),
                "Qpi:",
                np.round(root.Qpi, 2),
            )
