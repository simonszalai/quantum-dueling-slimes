...
class MCTS:
    def __init__(self, model, C=0.1, threshold=0.05, repeats=100, simulation_repeats=1, simulation_depth=2, use_habit=True):
        self.model = model
    ...
    def active_inference_mcts(self, obs):
        ...
        state_0, _, _ = self.model.encoder_net.encode(obs)

        # Important to use the mean here as we repeat it cfg.action_dim times
        root_node = Node(state=state_0[0], model=self.model, C=self.C, using_prior_for_exploration=self.using_prior_for_exploration)
        ...
        # P_action = self.model.habitual_net.predict_action(state_0)
        P_action = self.model.habitual_net.predict_action_from_obs(obs)
        if not isinstance(P_action, np.ndarray):
        ...
        if self.use_habit:
            ...
            if habitual_threshold > self.threshold:
                ...
                print("root_node.P_action", root_node.P_action)
                if cfg.replace_habit_with_baseline:
                    choosen_action = np.argmax(root_node.P_action)
                else:
